...
export function createDailyHandler() {
  return async (request: Request): Promise<Response> => {
    ...
    try {
      ...
      const guesses = await dailyGuesses(today, body.deviceId)
      if (guesses.some((guess) => guess.cod === answer.cod))
        return error(request, 'GAME_COMPLETE', 409)
      ...
      if (body.action === 'guess') {
        ...
        if (response.status === 409)
          return error(request, 'DUPLICATE_GUESS', 409)
        if (!response.ok)
          throw new Error(`Guess insert failed: ${response.status}`)
      ...
      const usedHints = await dailyHints(today, body.deviceId)
      const tier = usedHints.length + 1
      if (body.tier !== tier) return error(request, 'INVALID_HINT_TIER', 400)
      const text = hintText(hints, answer.cod, tier)
      if (!text) return error(request, 'INVALID_HINT_TIER', 400)
      if (!(await consumeAction(today, body.deviceId, 'hint', 20)))
        return error(request, 'RATE_LIMITED', 429)
      const response = await serviceRequest('daily_hints', {
        method: 'POST',
        headers: { prefer: 'return=minimal' },
        body: JSON.stringify({
          puzzle_date: today,
      ...
      if (!response.ok)
        throw new Error(`Hint insert failed: ${response.status}`)
